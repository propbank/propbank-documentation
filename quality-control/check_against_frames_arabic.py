...
def check_basic_formatting(targetfile):
    ...
    for index, eachline in enumerate(tbox):
        ...
        weirdargs = [x for x in allargs if not x in ["ARG0","ARG1", "ARG2","ARG3","ARG4","ARG5", "ARGM-CND", "ARGM-INS", "ARGM-LOC","ARGM-PRP", "ARGM-TMP", "ARGM-DSP", "ARGA", "ARGM-REC", "ARGM-MNR", "ARGM-ADV","ARGM-NEG","ARGM-EXT","ARGM-LVB","ARGM-PRR","ARGM-DIR","ARGM-ADJ","ARGM-GOL","ARGM-CXN","ARGM-CAU","ARGM-MOD","ARGM-PRD","ARGM-COM","ARGM-DIS"]]
        if len(weirdargs) > 0:
    ...
    for aline in [x.strip() for x in open(targetfile)]:
        ...
        if roleset+"%%"+fr in seenroles:
            ...
            if fr+".xml" in osl:
                ...
                for somers in y.find_all("frameset"):
                    if roleset == fr.replace("-v",".").replace("-n",".")+somers['id']:
                        rightroleset = somers
                        seenroles[roleset+"%%"+fr] = somers
        if rightroleset:
            argbox= [x.split("ARG")[1] for x in arguments if "-ARG" in x and x.split("ARG")[1].isdigit()]
            valids = [anyrole['argnum'] for anyrole in rightroleset.find_all('role')]
