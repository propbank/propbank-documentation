...
CHECKCONTIGUITYOFROLESETS = True
...
for afile in sorted(os.listdir(floc)):
	...
	rsbox = []
	if not chardet.detect(open(floc+afile,'rb').read())['encoding'] in ['utf-8','ascii']:
		print "error!!! not utf-8 encoding", afile
		print chardet.detect(open(floc+afile).read())
		print "converting to unicode...."
		#readfile = codecs.open(floc+afile,'r','utf-8')

		#r = unicode(readfile.read().decode(chardet.detect(open(floc+afile,'rb').read())['encoding']).encode('utf-8'))	
		#readfile.close()
		#writefile = codecs.open(floc+afile,'w','utf-8')
		#writefile.write(r)
